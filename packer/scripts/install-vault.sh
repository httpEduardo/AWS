...
function print_usage {
  ...
  echo
  echo -e "  --fix-cloudinit\t\t Disables DEBUG logging in cloudinit, which can reveal secrets in the log."
  echo 
...
function fix_cloudinit_log {
  sed -i s/level=DEBUG/level=INFO/ /etc/cloud/cloud.cfg.d/05_logging.cfg
}
